    ...
    <div className="flex-1 flex flex-col min-h-0 bg-transparent relative">
      ...
      <div className="h-16 bg-zinc-900/50 backdrop-blur-md flex justify-between items-center px-6 shrink-0 shadow-sm sticky top-0 z-10">
        ...
        >
          ...
          <span className="text-[10px] font-black text-zinc-900 relative z-10 uppercase tracking-widest whitespace-nowrap">Add Account</span>
        </button>
              ...
              {accounts.map((t) => (
                <div key={t.id} className="glass-card rounded-[2rem] p-5 sm:p-6 md:p-8 shadow-2xl relative group overflow-visible transition-all duration-300 hover:bg-zinc-900/60 flex flex-col justify-between">
                  <button
                     ...
                     className="absolute -top-3 -right-3 z-30 text-zinc-400 hover:text-rose-400 p-2 rounded-full bg-zinc-900 border border-white/10 shadow-2xl transition-all opacity-0 group-hover:opacity-100 active:scale-95"
                  ...
                  <div className="flex flex-col gap-5">
                    <div className="flex flex-col gap-1.5">
                      <span className="text-[10px] uppercase text-zinc-500 font-black tracking-[0.3em] text-center">Account Detail</span>
                      <input
                        type="text"
                        value={t.name}
                        ...
                        placeholder="Primary Holding..."
                      />
                    </div>

                    <div className="flex flex-col gap-1.5 p-4 sm:p-5 rounded-2xl bg-zinc-800/30 border border-white/5">
                      <span className="text-[10px] uppercase text-zinc-500 font-black tracking-[0.3em] text-center">Current Balance</span>
                      <div className="flex items-center justify-center">
                        <span className={`font-mono text-xl mr-1.5 ${t.balance >= 0 ? 'text-pink-500 italic' : 'text-rose-400'}`}>{symbol}</span>
                    ...
                    <div className="flex items-center justify-center gap-4 mt-1">
                      <div className="relative">
                         <select
                          ...
                          className="bg-zinc-800 border border-white/5 px-4 py-2 rounded-xl outline-none font-black text-[9px] uppercase text-zinc-300 appearance-none transition-all cursor-pointer focus:border-teal-400/30 pr-10"
                         >
                          {ACCOUNT_TYPES.map(type => (
                            <option className="bg-zinc-900 text-zinc-100" key={type} value={type}>{type}</option>
                          ))}
                        </select>
                        <div className="absolute right-3 top-1/2 -translate-y-1/2 pointer-events-none text-zinc-500 font-bold">
                          &darr;
                        </div>
                      </div>
                    </div>
