...
1. **חיבור read-only לברוקר** — הקטן מביניהם. מושך אחזקות ומחירים בלבד.
2. **משיכת נתונים מברוקר** — גם היסטוריית עסקאות.
3. **בנקאות פתוחה** — תנועות עו"ש אוטומטיות במקום ייבוא אקסל ידני.

**המתח שצריך לפתור לפני שמתחילים:** כל השלושה דורשים OAuth וטוקן חי,
וזה מתנגש חזיתית עם שתי דרישות היסוד — **100% אופליין** ו**אפס ענן**.
בישראל בנקאות פתוחה עוברת דרך ספק מורשה, כלומר גם צד שלישי וגם עלות
חודשית, מול "אפס תשלום" בבריף. הייבוא מאקסל (`bankImport.ts`) קיים
ועובד בדיוק בגלל זה.

כשנגיע לזה, השאלה הראשונה לאופק היא איזו מהדרישות מוותרים עליה.
