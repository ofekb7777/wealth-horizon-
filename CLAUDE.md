...
│   ├── he.ts zh.ts hi.ts es.ts ar.ts     חמש השפות האחרות
...
**שש שפות.** אנגלית היא ברירת המחדל, ולצדה סינית, הינדי, ספרדית, ערבית
ועברית. הבחירה מוצגת פעם אחת בהרצה הראשונה (`LanguagePicker`) וניתנת
לשינוי בהגדרות. עברית וערבית מפעילות RTL. **התרגום עובר דרך `useI18n()`
ולא דרך ייבוא ישיר** — ייבוא ברמת המודול נקבע פעם אחת בטעינה ולא היה
מתעדכן בשינוי שפה.
...
ל-logical properties. שלב 7 הפך את זה לשש שפות: אנגלית (ברירת מחדל),
סינית, הינדי, ספרדית, ערבית ועברית. עבודת ה-logical properties היא בדיוק
מה שאיפשר לערבית להיכנס בלי לגעת בפריסה.
...
האפליקציה בסינית בדפדפן אמיתי וסרקנו את ה-DOM אחרי טקסט לטיני — ביניהן
...
**פונטים:** רק Heebo ארוז (עברית + לטינית). לערבית, לסינית ולהינדי
מסתמכים על פונטי המערכת של אנדרואיד (Noto), כי אריזתם הייתה מוסיפה
מיליוני בתים ל-APK.
...
npm run test:i18n         # 21 בדיקות לשלמות ששת המילונים
npm run sweep:i18n        # מרנדר בסינית בדפדפן ומחפש טקסט שלא תורגם
